...
df = pl.read_parquet('/Users/aanyajain/Documents/GitHub/fs-data/FS-3/11222025/11222025_9.parquet')
df = df.fill_null(strategy="forward").fill_null(strategy="backward")
signal = df["TPERIPH_BL_TIRETEMP_1"].to_numpy()

fft = scipy.fft.fft(signal)
freqs = scipy.fft.fftfreq(len(signal), d=0.01)

attempts = [9, 9.5, 10, 10.5]
ffts = []
iffts = []
...
plt.xlabel("Attempt")
plt.ylabel("RMS")
plt.title("Tire Temp BL - RMS vs Attempt")
plt.show()
...
plt.title("Tire Temp BL - Frequency Spectrum")
plt.show()
