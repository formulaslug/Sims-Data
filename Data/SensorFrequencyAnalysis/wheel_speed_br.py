...
df = pl.read_parquet('/Users/aanyajain/Documents/GitHub/fs-data/FS-3/01112026/011026-18.parquet')
df = df.fill_null(strategy="forward").fill_null(strategy="backward")
signal = df["TPERIPH_BR_DATA_WHEELSPEED"].to_numpy()
...
plt.xlabel("Attempt")
plt.ylabel("RMS")
plt.title("Wheel Speed BR - RMS vs Attempt")
plt.show()
...
plt.title("Wheel Speed BR - Frequency Spectrum")
plt.show()
plt.scatter(freqs, np.log(np.abs(fft)), s=0.25)
plt.xlabel("Frequency (Hz)")
plt.ylabel("Magnitude")
