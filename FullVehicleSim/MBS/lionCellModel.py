...
def update_pack_voltage_template(prev_current: float, vehicle_state, params=None) -> float:
    ...
    # --- dt ---
    dt = float(getattr(vehicle_state, "stepSize", 1.0))

    # --- initialize battery memory states if missing ---
    if not hasattr(vehicle_state, "batt_v_rc"):
        vehicle_state.batt_v_rc = 0.0
    if not hasattr(vehicle_state, "batt_hyst"):
        vehicle_state.batt_hyst = 0.0
    if not hasattr(vehicle_state, "batt_temp_c"):
        vehicle_state.batt_temp_c = 25.0
    ...
    SOC = float(np.clip(vehicle_state.charge / 2.5, 0.0, 1.0))
    ...
    V_RC = float(vehicle_state.batt_v_rc * exp_factor + R1 * (1 - exp_factor) * prev_current)

    # --- Gaussian current from history array ---
    # Use the array from state; if missing, just use prev_current.
    hist = np.asarray(getattr(vehicle_state, "current_history", np.array([prev_current])), dtype=float)
    ...
    H = float(
        lambda_hyst * vehicle_state.batt_hyst
        + (1 - lambda_hyst) * prev_current
        + gauss_weight * I_gauss
    )

    # --- thermal update ---
    T_prev = float(vehicle_state.batt_temp_c)
    T = float(
   ...
   # --- cell terminal voltage ---
    ...
    vehicle_state._cell_voltage = V_cell

    vehicle_state.batt_v_rc = V_RC
    vehicle_state.batt_hyst = H
    vehicle_state.batt_temp_c = T

    return float(series_cells * V_cell)